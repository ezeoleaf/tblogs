...
docker-build:
	...
	docker build -t tblogs:build --build-arg=version=master -f Dockerfile.build .
	@echo "Done with docker build"

## docker-install: installs a local version of the app from docker build
docker-install:
	@echo "Installing..."
	docker create --name tblogs_build tblogs:build
	docker cp tblogs_build:/usr/local/bin/tblogs ~/.local/bin/
	$(eval INSTALLPATH = $(shell which ${APP}))
	@echo "${APP} installed into ${INSTALLPATH}"
	docker rm tblogs_build
